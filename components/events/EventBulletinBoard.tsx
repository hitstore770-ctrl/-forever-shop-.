"use client";

import { useState } from "react";
import { motion } from "framer-motion";
import { PinIcon } from "@/components/icons";
import RsvpForm from "@/components/events/RsvpForm";
import type { UpcomingEvent } from "@/lib/events-data";
import { fadeInUp, staggerContainer, hoverTilt } from "@/lib/motion-variants";

const ROTATION_DEG = [-2, 2, -1, 1];
const ACCENTS = ["bg-cream", "bg-copper-300", "bg-cream-dark", "bg-cream"];

function spotsLeftFor(event: UpcomingEvent, takenCount: number): number | null {
  const capacity = event.capacity ?? 0;
  return capacity > 0 ? Math.max(0, capacity - takenCount) : null;
}

// A physical corkboard of pinned event notes. Firestore-backed and editable
// from /admin, with an optional cover image. RSVP-enabled events show the
// remaining seat count and an "אישור הגעה" button that opens the capacity-
// checked RSVP modal. `taken` maps eventId → confirmed attendees.
export default function EventBulletinBoard({
  events,
  taken = {},
}: {
  events: UpcomingEvent[];
  taken?: Record<string, number>;
}) {
  const [rsvpEvent, setRsvpEvent] = useState<UpcomingEvent | null>(null);

  return (
    <div className="border-4 border-black bg-navy-950 p-6 shadow-brutal-lg sm:p-10">
      <motion.div
        initial="hidden"
        whileInView="show"
        viewport={{ once: true, amount: 0.2 }}
        variants={staggerContainer(0.1)}
        className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4"
      >
        {events.map((event, index) => {
          const takenCount = taken[event.id] ?? 0;
          const left = spotsLeftFor(event, takenCount);
          const isFull = left === 0;

          return (
            <motion.div
              key={event.id}
              variants={fadeInUp(ROTATION_DEG[index % ROTATION_DEG.length])}
              whileHover={hoverTilt}
              className={`relative border-4 border-black p-5 pt-8 shadow-brutal ${ACCENTS[index % ACCENTS.length]}`}
            >
              <span className="absolute -top-4 left-1/2 flex h-8 w-8 -translate-x-1/2 items-center justify-center rounded-full border-2 border-black bg-copper-500 text-navy-950 shadow-brutal">
                <PinIcon className="h-4 w-4" />
              </span>

              {event.imageUrl && (
                <div className="mb-4 overflow-hidden border-2 border-black">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={event.imageUrl} alt={event.title} loading="lazy" className="h-32 w-full object-cover" />
                </div>
              )}

              <p className="text-xs font-semibold tracking-widest text-copper-600 uppercase">{event.date}</p>
              <h3 className="mt-1 text-lg font-semibold text-navy-950">{event.title}</h3>
              <p className="mt-2 text-sm font-normal text-navy-700/80">{event.description}</p>

              {event.rsvpEnabled && (
                <div className="mt-4 border-t-2 border-navy-900/15 pt-3">
                  <p className="text-xs font-semibold text-navy-900/70 uppercase">
                    {event.capacity && event.capacity > 0 ? `${takenCount}/${event.capacity} מקומות` : "הרשמה פתוחה"}
                  </p>
                  {isFull ? (
                    <span className="mt-2 inline-block border-2 border-black bg-navy-900 px-3 py-1.5 text-xs font-semibold text-cream uppercase">
                      האירוע מלא
                    </span>
                  ) : (
                    <button
                      type="button"
                      onClick={() => setRsvpEvent(event)}
                      className="mt-2 border-2 border-black bg-copper-500 px-3 py-1.5 text-xs font-semibold text-navy-950 uppercase shadow-brutal transition-all hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-brutal-none"
                    >
                      אישור הגעה
                    </button>
                  )}
                </div>
              )}
            </motion.div>
          );
        })}
      </motion.div>

      {rsvpEvent && (
        <RsvpForm
          event={rsvpEvent}
          spotsLeft={spotsLeftFor(rsvpEvent, taken[rsvpEvent.id] ?? 0)}
          onClose={() => setRsvpEvent(null)}
        />
      )}
    </div>
  );
}
